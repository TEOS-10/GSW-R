...
Recently, new formulae for the calculation of seawater properties have been
sanctioned by international advisory bodies
\cite[]{ioc2010tite,millero2010hote}.  An important component is the Gibbs
Seawater (\GSW) toolbox, which is available for Matlab, C, and Fortran
\cite[]{mcdougall2011gswt,pawlowicz2012ahpo}. As of early 2015, versions were
being coded for other languages, and \gsw, the result for R, may be used
independently or within the more general \oce package \cite[]{kelley2014oaoo}.

Readers are assumed to be familiar with oceanographic processing, and at least
somewhat familiar with \GSW.  A good resource for learning more about \GSW is
the its website (\url{http://www.teos-10.org}), which provides technical manuals
for the Matlab version of \GSW
(\url{http://www.teos-10.org/pubs/gsw/html/gsw_contents.html}), along with white
papers and links to the growing literature on the topic.

The \gsw framework involves a series of wrappers that connect R with the C
version of the Gibbs Seawater library. This scheme yields high processing speed.
It also yields high reliability, by avoiding transliteration errors. (The
incorporation of \GSW check values into the package-building process also
...
provided on the \GSW website. This fits with the overall goal of \gsw, which is
to provide an interface to \GSW algorithms, on the assumption that users will
learn about the algorithms from the \GSW website, and learn about their
scientific basis and practical significance from the peer-reviewed literature
\cite[see e.g.][]{wright2011asds,mcdougall2012agaf}.

...
resource for learning more about \GSW is the its website
(\url{http://www.teos-10.org}), which provides technical manuals for the Matlab
version of \GSW (\url{http://www.teos-10.org/pubs/gsw/html/gsw_contents.html}),
along with white papers and links to the growing literature on the topic.

For practical reasons, \gsw does not adhere to strict SI units, but rather
incorporates oceanographic units, such as decibars (dbar) for pressure.  The
\gsw framework involves a series of wrappers that connect R with the C version
of the Gibbs Seawater library. This scheme yields high processing speed.  It
also yields high reliability, by avoiding transliteration errors. The
incorporation of \GSW check values into the package-building process also
enhances reliability.

\gsw documentation is minimal, amounting mainly to an explanation of function
arguments and return values, with hyperlinks to the \GSW reference documentation
providing the details. This fits with the overall aim of connecting
oceanographic analysts with the \GSW methodology, letting them learn about
technical details from the \GSW documentation, and about scientific issues from
the peer-reviewed literature \cite[see
e.g.][]{wright2011asds,mcdougall2012agaf,graham2013qtnp}.
